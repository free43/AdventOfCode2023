...
int main()
{
    ...
    auto is_special_char = [&s](int i, int k)
    {
        return !isdigit(s[i][k]) && s[i][k] != '.' && s[i][k] != 0;
    };
    int sum = 0;
    for(size_t i = 0; i < s.size(); i++)
    {
        for(int k = 0; k < numb_of_chars; ++k)
        {
            if(isdigit(s[i][k]))
            {
                ...
                if(k > 0)
                {
                    found_special_char |= is_special_char(i, k - 1);
                    if(i > 0)
                    {
                        found_special_char |= is_special_char(i - 1, k - 1);
                    }
                    if(i < s.size() - 1)
                    {
                        found_special_char |= is_special_char(i + 1, k - 1);
                ...
                for(; k < numb_of_chars; ++k)
                {
                    ...
                    if(i > 0)
                    {
                        found_special_char |= is_special_char(i - 1, k);
                    }
                    if(i < s.size() - 1)
                    {
                        found_special_char |= is_special_char(i + 1, k);
                ...
                found_special_char |= is_special_char(i, k);
                if(i > 0)
                {
                    found_special_char |= is_special_char(i - 1, k);
                }
