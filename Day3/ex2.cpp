...
int main()
{
    ...
    const int numb_of_chars = 10;*/
    unordered_map<int, pair<int, int>> m;
    auto is_special_char = [&s](int i, int k, vector<int>& vec)
    {
        if(s[i][k] == '*')
        {
            vec.emplace_back(i * numb_of_chars + k);
            return true;
        }
        return false;
    };
    ...
    for(size_t i = 0; i < s.size(); i++)
    {
        for(int k = 0; k < numb_of_chars; ++k)
        {
            if(isdigit(s[i][k]))
            {
                ...
                if(k > 0)
                {
                    found_special_char |= is_special_char(i, k - 1, pos);
                    if(i > 0)
                    {
                        found_special_char |= is_special_char(i - 1, k - 1, pos);
                    }
                    if(i < s.size() - 1)
                    {
                        found_special_char |= is_special_char(i + 1, k - 1, pos);
                ...
                for(; k < numb_of_chars; ++k)
                {
                    ...
                    if(i > 0)
                    {
                        found_special_char |= is_special_char(i - 1, k, pos);
                    }
                    if(i < s.size() - 1)
                    {
                        found_special_char |= is_special_char(i + 1, k, pos);
                ...
                found_special_char |= is_special_char(i, k, pos);
                if(i > 0)
                {
                    found_special_char |= is_special_char(i - 1, k, pos);
                }
